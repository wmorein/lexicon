...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "lexicon mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    } = transport
    {
        match supports_oauth_login(&url).await {
            ...
            Ok(false) => {}
            Err(_) => println!(
                "MCP server may or may not require login. Run `lexicon mcp login {name}` to login."
            ),
        }
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `lexicon mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: lexicon mcp remove {}", get_args.name);
